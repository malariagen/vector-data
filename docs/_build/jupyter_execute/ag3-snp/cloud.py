# Ag1000G phase 3 SNP data release - cloud data access guide

**14 January 2021**

## Introduction

This notebook provides information about how to access data from the [MalariaGEN Anopheles gambiae 1000 Genomes project (Ag1000G) phase 3 SNP data release](https://www.malariagen.net/data/ag1000g-phase3-snp) via Google Cloud. This includes sample metadata and single nucleotide polymorphism (SNP) calls.

Ag1000G phase 3 includes SNP calls for 3,081 individual mosquitoes, including 2,784 mosquitoes collected from natural populations in 19 countries. The map below provides an overview of the collection locations and the numbers of samples broken down by species. Further information about the samples and the studies which contributed them is available from the [contributing studies document](https://storage.googleapis.com/vo_agam_release/v3/ag1000g-phase3-contributing-studies.pdf).

<img src="ag3-map.png" alt="map" width="600"/>

Please note that data from Ag1000G phase 3 are being released prior to publication. You are free to use these data for any purpose, but the data are under a publication embargo described further in the [Ag1000G terms of use](https://www.malariagen.net/data/terms-use/ag1000g-terms-use). If you have any questions about the terms of use, please email data@malariagen.net.
...
This guide is written as a Jupyter notebook. It illustrates how to read data directly from the cloud, without having to first download any data locally. This notebook can be run from any computer, but will work best when run from a compute node within Google Cloud, because it will be physically closer to the data and so data transfer is faster. For example, this notebook can be run via [Google Colab](https://colab.research.google.com/) or [Google Cloud Datalab](https://cloud.google.com/datalab/docs) which are interactive computing service running in the cloud. Here is a link to [open this notebook in Google Colab](https://colab.research.google.com/github/malariagen/vector-public-data/blob/master/docs/ag3-snp-download-guide.ipynb).

This guide covers accessing SNP calls and associated sample metadata only. If you wish to access raw sequence reads or sequence read alignments you will need to download data locally, see the [Ag1000G phase 3 SNP download guide @@TODO link](@@TODO).

## Data hosting

All data required for this notebook is hosted on Google Cloud Storage (GCS). Sample metadata in CSV format and SNP calls in Zarr format are hosted on GCS in the `vo_agam_release` bucket, which is a multi-region bucket located in the United States. All data hosted in GCS are publicly accessible and do not require any authentication to access. 

## Setup

Running this notebook requires the following Python packages to be installed: numpy, dask, zarr, gcsfs, fsspec, scikit-allel.

These packages can be installed via pip or conda. E.g.:

!pip install -q numpy dask[array] zarr gcsfs fsspec scikit-allel
...
Once installed, data access via GCS is set up via the following code:
...
To see which sample sets are available, load the sample set manifest into a [pandas](https://pandas.pydata.org/) dataframe:
...
E.g., load sample metadata for the AG1000G-BF-A and AG1000G-BF-B sample sets:
...
Pandas can be used to explore and query the sample metadata in various ways. E.g., here is a summary of the numbers of samples by sample set, collection location, year and species:

df_samples.groupby(["sample_set", "country", "year", "species"]).size()

Note that samples within a sample set may belong to different species. For convenience, we have made a species call for all samples in Ag1000G phase 3, using the genomic data. Calling species is not always straightforward, and we have used two different methods for species calling, ancestry informative markers (AIM) and principal components analysis (PCA). When loading the sample metadata, the AIM species calls are included by default. The results of these two different methods generally agree, although there are some populations where results are different, particularly in Guinea-Bissau and The Gambia. If you have any questions about how to interpret these species calls, please get in touch.
...
df_samples = ag3.sample_metadata(cohort=cohort)
...
For further information about the Ag1000G project, please visit the [Ag1000G website](https://www.malariagen.net/ag1000g).
...
* [Ag1000G phase 3 contributing studies](https://storage.googleapis.com/vo_agam_release/v3/ag1000g-phase3-contributing-studies.pdf) - Provides information about the studies that collected and contributed specimens to Ag1000G phase 3, including collection information, related publications and key people involved.
* [Ag1000G phase 3 SNP calling methods](https://storage.googleapis.com/vo_agam_release/v3/ag1000g-phase3-snp-calling-methods.pdf) - Provides information about the methods used for sequencing, sequence read alignment, SNP calling, and quality control.
* [Ag1000G phase 3 SNP data release - data download guide @@TODO](@@TODO) - A guide to downloading data from this data release and accessing them from a local file system.